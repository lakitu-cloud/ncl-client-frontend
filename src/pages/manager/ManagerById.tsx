...
export const ManagerDetailPage = () => {
  ...
  // const totalUnits = manager.transaction.reduce((sum, m) => sum + (m.units || 0), 0);
  const activeMeters = manager.meters.filter(
    (m) => m.status === "active",
  ).length;
    ...
    <>
      ...
      <div className="flex h-screen">
        ...
        <motion.aside
          ...
          className="w-80 border-r border-gray-400 dark:border-gray-700 flex flex-col overflow-y-auto">
          {/* Profile Section */}
            ...
            <div className="flex flex-col items-center text-center">
              <div className="relative mb-5">
                <motion.div
                  ...
                  transition={springTransition}
                  className="w-32 h-32 uppercase font-oswald bg-gradient-to-br from-blueTheme to-blue-800 rounded-full flex items-center justify-center text-white text-5xl font-bold shadow-xl">
                  {manager.name
                    .split(" ")
                    .map((n) => n[0])
                    .join("")
              ...
              <h2 className="text-2xl uppercase font-oswald font-bold text-blackText dark:text-whiteText">
                {manager.name}
              </h2>
              <p className="text-blueTheme dark:text-blue-600 uppercase font-poppins font-bold mt-1">
                Sales Manager
              </p>
            </div>
          </div>
          ...
          <div className="p-4">
            ...
            <dl className="space-y-4 text-sm text-blackText dark:text-whiteText font-poppins">
              <div className="flex item-center justify-between">
                ...
                <dd className="font-medium mt-1"><CountUp value={Number(totalSubscribers ?? 0)} /></dd>
              </div>

              <div className="flex item-center justify-between">
                <dt className="font-medium">Total Meters</dt>
                <dd className="font-medium mt-1"><CountUp value={Number(activeMeters ?? 0)} /></dd>
              </div>
              <div className="flex item-center justify-between">
                <div className="font-medium">Volume Consumed</div>
                <div><CountUp value={Number(manager.metrics.collectiveVolume ?? 0)} /></div>
              </div>
            </dl>
          </div>
        </motion.aside>
        ...
        <motion.main
          ...
          <div className="max-w-full px-12 py-8">
            ...
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-2xl font-oswald font-bold text-blackText dark:text-whiteText uppercase">
                Manager Details
              </h1>
            ...
            <div className="mt-8">
              <AnimatePresence mode="wait">
                ...
                >
                  {activeTab === "meters" && (
                    <MetersTable meters={manager.meters} />
                  )}

                  {activeTab === "subscribers" && (
                    <SubscribersTable subscribers={manager.subscribers} />
                  )}
